...
<body class="text-biru">
    <section>
        <div class="fixed w-60">
            ...
            {{-- Nav --}}
            <div
                class="absolute  backdrop-blur-3xl bg-slate-200 bg-opacity-70 h-screen z-10 top-0 p-4 -translate-x-full duration-300 md:translate-x-0 md:relative">
                ...
                <ul class=" w-full mt-10 text-biru font-semibold ">
                    <li class="mb-2">
                        <a href="{{ route('tamu.dashboard') }}"
                            class="block p-4 rounded-md hover:bg-biru hover:text-white space-x-2 transition duration-300 ease-in-ot {{ Request::is('tamu/dashboard*') ? 'active' : '' }}">
                    ...
                    <li class="mb-2">
                        <a href="{{ route('dataTamu') }}"
                            class="block p-4 rounded-md hover:bg-biru hover:text-white  space-x-2 transition duration-300 ease-in-out">
                            <i class="fa-solid fa-users {{ Request::is('dataTamu*') ? 'active' : '' }}"></i>
                            <span>Data Tamu</span>
                        </a>
                    </li>
                    <li class="mb-2">
                        <a href="{{ route('riwayat') }}"
                            class="block p-4 rounded-md hover:bg-biru hover:text-white  space-x-2 transition duration-300 ease-in-out">
                            <i class="fa-solid fa-clock-rotate-left {{ Request::is('riwayat*') ? 'active' : '' }}"></i>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            sidebarOpen.addEventListener("click", function() {
                ...
                body.addEventListener("click", function(event) {
                    // Mendapatkan element yang diklik
                    var clickedElement = event.target;

                    // Menutup sidebar jika area di luar sisi bar diklik dan bukan pada elemen yang tidak terisi atau kosong
                    if (!sidebar.contains(clickedElement) && clickedElement !== sidebarOpen &&
